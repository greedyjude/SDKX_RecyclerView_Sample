...
public class VerticalGridLayoutAdapter extends RecyclerView.Adapter<VerticalGridLayoutAdapter.ViewHolder> {
    @NonNull
    @NotNull
    @Override
    public ViewHolder onCreateViewHolder(@NonNull @NotNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(viewType, parent, false);
        if (viewType == R.layout.vertical_grid_image) {
    ...
    public int getItemViewType(int position) {
        if (position % 5 == 0) {
            return R.layout.vertical_grid_ad;
        } else {
            return R.layout.vertical_grid_image;
        }
    }
